...
pub fn get_content(page: &str) -> Option<String> {
    ...
    let candidate_sel = Selector::parse("span").ok()?;
...
fn clean_text(raw: &str) -> String {
    ...
    fn normalize_spacing(s: &str) -> String {
        ...
        while i < chars.len() {
            ...
            // opening parenthesis - remove any space immediately after
            if c == '(' || c == '[' || c == '{' {
                out.push(c);
                i += 1;
                while i < chars.len() && chars[i].is_whitespace() {
            ...
            // whitespace handling
            if c.is_whitespace() {
                // if next is a quote, decide whether to keep a space
                if i + 1 < chars.len() && (chars[i + 1] == '"' || chars[i + 1] == '\'') {
                    // check last pushed non-space char
                    if let Some(last) = out.chars().rev().find(|ch| !ch.is_whitespace()) {
                        if ['(', '[', '{'].contains(&last) {
                            // don't put space between opening paren and quote
                            i += 1;
                            continue;
                        } else {
                            // ensure exactly one space before quote
                            if out
                                .chars()
                                .last()
            ...
            if c == '"' {
                if !in_double {
                    // opening quote: ensure space before if needed
                    if out
                        .chars()
                        .last()
...
mod tests {
    ...
    fn test_grok_punctuation_normalization() {
        ...
        assert!(!extracted.contains(" ,"));
        assert!(!extracted.contains(" ."));
        // ensure quoted titles appear (spacing validated visually)
        assert!(
